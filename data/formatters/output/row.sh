...
format_output_row() {
  ...
  if ! formatted_player_name=$(strip_leading_and_trailing_double_quotes "$1"); then fail "Failed to format player name\n"; fi

  local formatted_status
  if ! formatted_status=$(strip_leading_and_trailing_double_quotes "$3"); then fail "Failed to format player status\n"; fi

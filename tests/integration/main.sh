...
main() {
  ...
  "${executable_script_path}" 2023 05 06 || fail "Could not calculate points for May 6th, 2023"

  local -r jq_executable_path="${file_directory}/../../.dependencies/bin/jq"
  "${executable_script_path}" 2023 05 06 "${jq_executable_path}" || fail "Could not calculate points for May 6th, 2023"
}
